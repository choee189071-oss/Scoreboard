...
def render_hybrid_section_workflow(
    ...
    with st.expander(f"Hybrid Source Resolver — {title}", expanded=expanded):
        st.info("Flow: AI structured JSON extraction first → regex backup/debug only → analyst review/approval → calculator/scorecard sync.")
        if subtitle:
            st.caption(subtitle)
        ...
        with c1:
            run_scan = st.button("Run AI-first Resolver", key=f"{key_prefix}_{section_id}_run")
        with c2:
            force_ai = st.button("Run Regex Backup / Debug", key=f"{key_prefix}_{section_id}_force_ai")
        with c3:
            st.caption("AI is attempted first when an API key is available. Regex is kept only as backup/debug.")
        ...
        if ai_extracted:
            ...
            ai_label = "No"
        summary_cols[5].metric("AI Extraction", ai_label)
